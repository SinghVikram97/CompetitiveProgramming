...
void mergeArrays(ll a[],ll left[],ll right[],int leftSize,int rightSize)
{
    int i=0,j=0,k=0;
...
void mergeSort(ll a[],int n) /// Here n is no of elements
{
    if(n==1)
    {
        return;
    ...
    mergeSort(right,n-mid-1);
    /// Merge these 2 sorted arrays
    mergeArrays(a,left,right,mid+1,n-mid-1);
}
int main()
{
    int n;
