https://drive.google.com/open?id=1Ku8MgxXCg6GVeJZD56PAsQFO37mSA7Lm
#include<bits/stdc++.h>
using namespace std;
template<typename T>
class Graph{
...
template<typename T>
bool Graph<T>::bellmanFord(T sourceNode)
{
    /// Distance array
    ...
    /// Do one more iteration and see if any update occurs
   for(auto it=adjList.begin();it!=adjList.end();it++)
    {
      for(auto it1=adjList[it->first].begin();it1!=adjList[it->first].end();it1++)
      {
...
int main()
{
    ...
    int sourceNode;
    cin>>sourceNode;
    if(g.bellmanFord(sourceNode))
    {
        cout<<"There is negative cycle that is reachable from source vertex"<<endl;
    }
    else
    {
